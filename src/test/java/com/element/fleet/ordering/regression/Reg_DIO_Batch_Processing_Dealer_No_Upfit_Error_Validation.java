 ...
 */
public class Reg_DIO_Batch_Processing_Dealer_No_Upfit_Error_Validation extends BaseWebDriver {
	...
	@Documentation(step = "Navigate to Batch Ordering Page and create/upload batch order", expected = "User should be abe to navigate to Batch Ordering Page and create/upload batch order")
	...
	@Documentation(step = "Read Errors in XLSX file downloaded and add them to a list", expected = "Error should be displayed when Data is not correct")
	public void readErrorFile() throws Throwable {
		OrderingBOBatchProcessingPage.getActualErrorColumn();
	}
	
	@Test(dependsOnMethods = "readErrorFile")
	@Documentation(step = "Move expected error file from resources folder to target folder", expected = "File should be moved successfully")
	public void moveErrorFile() throws Throwable {
		OrderingBOBatchProcessingPage.moveErrorMessagesFileToTarget(this.getClass().getSimpleName());
	}
	
	@Test(dependsOnMethods = "moveErrorFile")
