 ...
 */
public class Reg_MLO_External_User_Approve_Order extends BaseWebDriver {
	@BeforeClass
	...
	@Documentation(step = "Navigate to MAM queue screen, ORD-22224", expected = "Manager approval queue screen should display" )
	...
	@Documentation(step = "Expected Approver  with External user role logs and approves the order", expected = " Order should be approved by Approver User and order should be removed from list")
	public void verifyExpectedApproverCanAppproveOrder() throws Throwable {
		OrderingManagerApprovalQueuePage.approveSingleOrderNotFinal();
		OrderingHomePage.verifyLogOutFunctionality();
	}
	...
	@Documentation(step = "Super users validates that approval submitted by expected approver is shown, ORD-22215", expected = "Order should show expected approver as complete")
	public void verifyOrdersWereApprovedByExpectedApprover() throws Throwable {
	OrderingManagerApprovalMaintenancePage.verifyExpectedApproverApprovedTheOrder();
	}
	
	@Test(dependsOnMethods = "verifyOrdersWereApprovedByExpectedApprover")
