...
import com.element.fleet.ordering.page.OrderingBOMainframeBridgingQueuePage;
 ...
 */
public class Reg_OEM_Chrysler_OutBound extends BaseWebDriver {
	...
	@Documentation(step = "Go to mainframe bridging queue and verify order", expected = "Order should be visibled in mainframe bridging queue")
	public void goToMainframeBridgingQueueAndVerifyLogNumber() throws Throwable {
		OrderingBOMainframeBridgingQueuePage.goToMainFrameBridgingQueueAndSearch();
		OrderingBOMainframeBridgingQueuePage.verifyLogNumberPresent();
	}
	
	@Test(dependsOnMethods = "goToMainframeBridgingQueueAndVerifyLogNumber")	
	@Documentation(step = "Go to Order transmission queue and verify order", expected = "Order should be visibled in order transmission queue")
	public void goToOrderTransmissionQueueAndVerifyLogNumber() throws Exception {
		OrderingBOOrderTransmissionQueuePage.goToOrderTransmissionQueueAndSearch();
		OrderingBOOrderTransmissionQueuePage.verifyLogNumberPresent("Visible");
	}
	
	@Test(dependsOnMethods = "goToOrderTransmissionQueueAndVerifyLogNumber")
	@Documentation(step = "Go to Scheduler BMT", expected = "Scheuler BMT should be opened")
	public void goToSchedulerPage() throws Exception {
	...
	@Documentation(step = "Go to Order Transmission queue and verify order", expected = "Order should be invisible in Order Transmission queue")
	public void goToOrderTransmissionQueuePageSearchAndVerifyLogNumber() throws Exception {
		OrderingBOOrderTransmissionQueuePage.goToOrderTransmissionQueueAndSearch();
		OrderingBOOrderTransmissionQueuePage.verifyLogNumberPresent("Invisible");
	}
	
	@Test(dependsOnMethods = "goToOrderTransmissionQueuePageSearchAndVerifyLogNumber")	
	@Documentation(step = "Verify log number in Win Scp and verify order status", expected = "Log number should be present in Win Scp and order status should be changed")
	public void verifyLogNumberInWinScpAndVerifyOrderStatusOnMaitenancePage() throws Throwable {
		OrderingBOOrderTransmissionQueuePage.verifyLogNumberInWinScpServer();
		OrderingBOOrdMaintPage.verifyOrderStatus();
	}
	
	@Test(dependsOnMethods = "verifyLogNumberInWinScpAndVerifyOrderStatusOnMaitenancePage")
	@Documentation(step = "click on Logout button", expected = "Application should be logged out")
