 ...
 */
public class Reg_PO_Factory_Order_Cancel_Factory_Order extends BaseWebDriver {
	...
	@Documentation(step = "Validate send Po Now button and Cancel PO Button", expected = "Send PO Now and Cancel PO button should be displayed for upfit")
	public void validateSendPONowAndCancelPOButtons() throws Exception {
		OrderingBOOrdMaintPage.goToUpfitSection();
		OrderingBOOrdMaintPage.clickUpfitLineItem();
		OrderingBOOrdMaintPage.verifySendPONowButtonIsDisplayed();
		OrderingBOOrdMaintPage.verifyCancelPOButtonIsDisplayed();
	}
	
	@Test(dependsOnMethods = "validateSendPONowAndCancelPOButtons")
	@Documentation(step = "Validate user is able to cancel factory Order", expected = "User should be able to cancel factory order")
	public void cancelFactoryOrder() throws Exception {
		OrderingBOOnOrderQueuePage.orderDetailsPageSectionClick("General Order");
		OrderingBOOrdMaintPage.cancelFactoryOrder();
	}
	
	@Test(dependsOnMethods = "cancelFactoryOrder")
	@Documentation(step = "Validate order status change", expected = "Order status should change to Purchase Order Cancelled")
	public void validateOrderStatusChange() throws Exception {
		OrderingBOOrdMaintPage.verifyChangedOrderStatus("ZZ0010 - Purchase Order Cancelled");
	}
	
	@Test(dependsOnMethods = "validateOrderStatusChange")
	@Documentation(step = "Validate Send PO Now and Cancel Po Buttons", expected = "Send PO Noe and Cancel PO button should not be displayed for Cancelled Order")
	public void validateSendPONowAndCancelPO() throws Exception {
		OrderingBOOrdMaintPage.goToUpfitSection();
		OrderingBOOrdMaintPage.clickUpfitLineItem();
		OrderingBOOrdMaintPage.verifySendPONowButtonIsNotDisplayed();
		OrderingBOOrdMaintPage.verifyCancelPOButtonIsNotDisplayed();
	}
	
	@Test(dependsOnMethods = "validateSendPONowAndCancelPO")
	@Documentation(step = "Validate PO Status on order cancellation", expected = "Send PO Noe and Cancel PO button should not be displayed for Cancelled Order")
	public void validatePOStatus() throws Exception {
		OrderingBOOrdMaintPage.validateCurrentPOStatus("Purchase Order Cancelled");
	}
	
	@Test(dependsOnMethods = "validatePOStatus")
